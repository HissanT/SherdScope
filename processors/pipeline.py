...
class MaskExtractor:
    ...
    def _save_metadata(self,
        ...
        """Save extraction metadata without discarding reviewed metadata.

        Existing user columns are carried forward by card filename. Linkage-owned
        ...
        if info_path.exists():
            try:
                old_info = pd.read_csv(info_path, dtype=str, keep_default_na=False)
            except Exception as exc:
                print(f"Warning: could not preserve existing mask metadata: {exc}")
        if annots_path.exists():
            try:
                old_annots = pd.read_csv(annots_path, dtype=str, keep_default_na=False)
            except Exception as exc:
                print(f"Warning: could not compare existing card geometry: {exc}")

        df = pd.DataFrame(metadata, columns=["file", "mask_file"])
        if px_per_cm_map:
            df['px_per_cm'] = df['mask_file'].map(px_per_cm_map).fillna('')
        new_annots = pd.DataFrame(annotations, columns=["bbox", "mask_file"])

        if old_info is not None and 'mask_file' in old_info.columns:
            old_info = old_info.copy()
            old_info['_key'] = old_info['mask_file'].map(mask_stem)
            old_by_key = old_info.drop_duplicates('_key', keep='last').set_index('_key')
            old_bbox = {}
            if old_annots is not None and {'mask_file', 'bbox'}.issubset(old_annots.columns):
                old_bbox = {mask_stem(row['mask_file']): parse_bbox(row['bbox'])
                            for _, row in old_annots.iterrows()}
            new_bbox = {mask_stem(row['mask_file']): parse_bbox(row['bbox'])
                        for _, row in new_annots.iterrows()}
            for column in old_info.columns:
                if column in {'_key', 'file', 'mask_file', 'px_per_cm'}:
                    continue
                values = []
                for key in df['mask_file'].map(mask_stem):
                    value = old_by_key.at[key, column] if key in old_by_key.index else ''
                    if column in ALL_LINKAGE_STORAGE_COLUMNS and old_bbox.get(key) != new_bbox.get(key):
                        value = ''
                    values.append(value)
                df[column] = values

        df.to_csv(info_path, index=False)
        new_annots.to_csv(annots_path, index=False)
        invalidate_linkage_for_card_changes(output_folder, old_annots, new_annots)
